...
function registrar(){
	var result = function_ajax({
		...
		'seleccionado': $("#seleccionadoagg").val(),
		'estado':'1'
}	,'../controller/direccionController.php').then(function(result){
	if(result=="1"){
		alert_success();
		ver_registros();
		$("#nombreagg").val("");
...
function cargar_datos(id_direccion,id_usuarios,nombre,direccion,telefono,latitude,longitude,seleccionado){
	$("#id").val(id_direccion);
	$("#nombre").val(nombre);
...
function modificar(){
	...
	var seleccionado =  $("#seleccionado").val();
	Swal.fire({
