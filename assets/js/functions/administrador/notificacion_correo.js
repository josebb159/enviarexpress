...
function registrar(){
	var result = function_ajax({
		...
		'accion': $("#accionagg").val(),
		'nombre': $("#nombreagg").val(),
		'estado':'1'
}	,'../controller/notificacion_correoController.php').then(function(result){
	if(result=="1"){
		alert_success();
		ver_registros();
		$("#correoagg").val("");
...
function cargar_datos(id_notificacion_correo,correo,accion,nombre){
	$("#id").val(id_notificacion_correo);
	$("#correo").val(correo);
	$("#accion").val(accion);
	$("#nombre").val(nombre);
}
...
function modificar(){
	var id =  $("#id_notificacion_correo").val();
