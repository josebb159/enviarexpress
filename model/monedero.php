...
class monedero {
	...
	public function registrar_monedero($id='204',$valor,$estado){
	$estado_defaul = 1;
	$sql = "INSERT INTO `monedero`(`estado`,`valor`) VALUES (:estado,:valor)";
	$reg = $this->conexion->prepare($sql);
	$reg->execute(array(':estado' => $estado_defaul,':valor' => $valor));
	return 1;
	}
	...
	public function modificar_monedero($id, $estado,$valor){
	$sql = "UPDATE `monedero` SET `estado`=:estado ,`valor`=:valor WHERE id_monedero=:id";
	$reg = $this->conexion->prepare($sql);
	$reg->execute(array(':id' => $id, ':estado' => $estado,':valor' => $valor));
	}
