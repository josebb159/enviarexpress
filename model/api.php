...
public function obtener_domiciliario($uid){
   ...
   // $sql = "SELECT  * FROM usuarios where usuario='jose' and contrasena='123' and estado=1";
    ...
    public function get_orden_on_proccess($uid){
        ...
        $conexion = new Conexion();
        $sql = "SELECT orden.*, temporaluser.nombre as client_nombre, temporaluser.telefono as client_telefono ,(SELECT comercios.nombre from comercios WHERE comercios.id_user=orden.id_tienda) as tienda,  0 as distancia, direccion.telefono as tele_direccion, (SELECT comercios.latitude from comercios WHERE comercios.id_user=orden.id_tienda)  as latitude, (SELECT comercios.longitude from comercios WHERE comercios.id_user=orden.id_tienda) as longitude, (select usuarios.nombre from usuarios where usuarios.id=orden.id_cliente) as cliente FROM orden left join direccion on direccion.id_direccion=orden.id_entrega left join usuarios on usuarios.id=orden.id_domiciliario left join temporaluser on temporaluser.id=orden.id_user_external where orden.status_orden_envio<3 and orden.recogido=1 and orden.orden_list!=0 and usuarios.uid='".$uid."'";
      // $sql = "SELECT  * FROM usuarios where usuario='jose' and contrasena='123' and estado=1";
        $reg = $conexion->prepare($sql);
